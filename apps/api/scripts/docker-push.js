...
function getDockerDaemonHint(errorOutput) {
  const output = String(errorOutput || '');

  if (output.includes('.docker/run/docker.sock')) {
    return [
      'Docker daemon is not reachable at the current socket.',
      'Start Docker Desktop, or switch to a working Docker context before retrying.',
      'Verify with: docker context ls && docker info',
    ].join('\n');
  }

  if (output.includes('/var/run/docker.sock')) {
    return [
      'Docker daemon is not reachable at /var/run/docker.sock.',
      'Start your Docker runtime (Docker Desktop, Colima, OrbStack, etc.) before retrying.',
  ...
  return [
    ...
    'Start your Docker runtime and verify the active context before retrying.',
    'Verify with: docker context ls && docker info',
  ].join('\n');
}

function ensureDockerIsReady(colors) {
  try {
    execSync('docker --version', { stdio: 'pipe' });
  } catch {
    console.error(`${colors.red}[ERROR] Docker CLI is not installed or not available in PATH${colors.reset}`);
    console.error(`${colors.yellow}[HINT] Install Docker and verify with: docker --version${colors.reset}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`${colors.red}[ERROR] Docker daemon is not available${colors.reset}`);
    console.error(`${colors.yellow}[HINT] ${getDockerDaemonHint(combinedOutput)}${colors.reset}`);
    process.exit(1);
  }
}
...
async function main() {
  ...
  if (typeof packageVersion !== 'string' || packageVersion.trim() === '') {
    console.error(`${colors.red}[ERROR] package.json is missing a valid version field${colors.reset}`);
    process.exit(1);
  }
